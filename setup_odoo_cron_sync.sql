-- Setup pg_cron jobs for automatic Odoo "last 25" sync
-- Run inside Supabase SQL editor after configuring app.supabase_* settings
...
BEGIN
    ...
    IF service_key IS NULL OR service_key = '' THEN
        RAISE EXCEPTION 'Service role key not configured. Please call setup_sync_config first.';
    END IF;
...
SELECT 
    ...
    command
FROM cron.job
WHERE jobname LIKE 'odoo-last25-sync%';
